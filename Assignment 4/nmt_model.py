...
class NMT(nn.Module):
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # print(f'enc_hoddens_proj has size {enc_hiddens_proj.size()}, unsqueeze dec hidden {dec_hidden.unsqueeze(2).size()}')
        e_t = (enc_hiddens_proj @ dec_hidden.unsqueeze(2)).squeeze(2)
        # print(f'e_t has size {e_t.size()}')
        ...
        # print(f'alpha_t has size {alpha_t.unsqueeze(1).size()}, enc_hiddens has size {enc_hiddens.size()}')
        a_t = (alpha_t.unsqueeze(1) @ enc_hiddens).squeeze(1)
        # print(f'a_t has size {a_t.size()}')
        # 3
